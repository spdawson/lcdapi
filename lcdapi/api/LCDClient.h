...
class LCDClient : public LCDElement
{
 ...
 public:
   ...
   * @param menu_event The menu event type which is to be handled. The following is a list of events available for each type of LCDMenu or LCDMenuWidget followed by the value parameter that may be returned by your handler's operator() function (NOTE: this list may be incomplete). LCDMenu: enter/leave. LCDMenuAction: select. LCDMenuAlpha: enter/leave/update (value: new value - only on update). LCDMenuCheckbox: update (value: on/off/grey). LCDMenuIP: enter/leave/update (value: new value - only on update). LCDMenuNumeric: enter/leave/update (value: new value - only on update). LCDMenuRing: update (value: new value of slider). LCDMenuSlider: enter/leave/plus/minus (value: new value - only on plus/minus).
